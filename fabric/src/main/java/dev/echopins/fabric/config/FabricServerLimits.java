 ...
 *
 * <p>Values are read once at load and held, because Fabric has no config-reload event to hook.
 * {@code /echopins admin reload} re-reads the file, which is the same affordance the NeoForge
 * build offers.
 */
public final class FabricServerLimits implements ServerLimits {

    private static volatile FabricServerLimits instance = new FabricServerLimits(
            FabricConfig.load(FabricConfig.Files_.SERVER));
    ...
    private FabricServerLimits(FabricConfig c) {
    ...
    /** Re-reads the file. Wired to {@code /echopins admin reload}. */
    public static void reload() {
        instance = new FabricServerLimits(FabricConfig.load(FabricConfig.Files_.SERVER));
    }
    ...
    public int recordingSessionTimeoutSeconds() {
        return recordingSessionTimeoutSeconds;
    }

    @Override
    public boolean suppressProximityBroadcastWhileRecording() {
        return suppressProximity;
    }

    @Override
    public int maxPinsPerPlayer() {
        return maxPinsPerPlayer;
    }
    ...
    public double playbackAudioDistance() {
        return playbackAudioDistance;
    }

    @Override
    public int maxSyncedPinsPerPlayer() {
        return maxSyncedPinsPerPlayer;
    }
    ...
    public int orphanCleanupIntervalMinutes() {
        return orphanCleanupIntervalMinutes;
    }

    @Override
    public boolean operatorBypassLimits() {
        return operatorBypassLimits;
    }
