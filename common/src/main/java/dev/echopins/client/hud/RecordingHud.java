...
public final class RecordingHud implements LayeredDraw.Layer {
    ...
    public void render(GuiGraphics graphics, DeltaTracker deltaTracker) {
        ...
        int x = anchorX(screenWidth, panelWidth) + ClientSettings.Holder.get().hudOffsetX();
        int y = anchorY(screenHeight) + ClientSettings.Holder.get().hudOffsetY();
        ...
        if (ClientSettings.Holder.get().highContrastRecordingIndicator()) {
    ...
    private void drawRecordDot(GuiGraphics graphics, int x, int y) {
        boolean solid = true;
        if (!ClientSettings.Holder.get().reduceMotion()) {
    ...
    private static int anchorX(int screenWidth, int panelWidth) {
        return switch (ClientSettings.Holder.get().hudPosition()) {
