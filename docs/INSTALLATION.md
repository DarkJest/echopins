...
| Loader | Forge 47.4.0+, NeoForge 47.1.0+, **or** Fabric Loader 0.19.3+ |
| Java | 17 |
| Required mod | Simple Voice Chat for 1.20.1, API version 2.6.20 or newer |
...
| Loader | File |
|---|---|
| Forge | `echopins-forge-<version>+mc1.20.1.jar` |
| NeoForge | `echopins-neoforge-<version>+mc1.20.1.jar` |
| Fabric | `echopins-fabric-<version>+mc1.20.1.jar` |
...
1. Install Forge, NeoForge or Fabric Loader for Minecraft 1.20.1.
2. On Fabric, also download **Fabric API** for 1.20.1.
3. Download Simple Voice Chat for 1.20.1 **for your loader**.
...
1. Install Forge 47.4.x, NeoForge 47.1.x or Fabric Loader for Minecraft 1.20.1.
...
3. Start the server once. This generates the config file: `config/echopins-server.toml` on
   Forge/NeoForge, `config/echopins-server.json` on Fabric. The options and their defaults are
...
- Ship `config/echopins-server.toml` (Forge/NeoForge) or `config/echopins-server.json` (Fabric) with
