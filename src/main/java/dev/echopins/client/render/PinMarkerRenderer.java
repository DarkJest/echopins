...
public final class PinMarkerRenderer {
    ...
    public static void render(PoseStack poseStack, MultiBufferSource.BufferSource buffers, Camera camera) {
        if (!EchoPinsClientConfig.SHOW_MARKERS.get()) {
            return;
        }
        ...
        double maxDistance = Math.min(
                EchoPinsClientConfig.MARKER_RENDER_DISTANCE.get(),
        ...
        int limit = Math.min(visible.size(), EchoPinsClientConfig.MAX_RENDERED_MARKERS.get());

        float opacity = EchoPinsClientConfig.MARKER_OPACITY.get().floatValue();
        float scale = EchoPinsClientConfig.MARKER_SCALE.get().floatValue();
    ...
    private static void drawMarker(PoseStack poseStack, MultiBufferSource buffers, PinSummary pin,
        ...
        if (!EchoPinsClientConfig.REDUCE_MOTION.get() && pin.unread()) {
    ...
    private static boolean shouldSeeThrough(double distance) {
        return switch (EchoPinsClientConfig.OCCLUSION_MODE.get()) {
